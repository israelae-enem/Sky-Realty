...
export const propertyColumns: ColumnDef<any>[] = [
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex items-center gap-3">
          {img ? (
            <Image src={img} alt={p.title || "property"} width={64} height={48} className="rounded-md object-cover" />
          ) : (
            <div className="w-14 h-10 bg-gray-200 rounded-md flex items-center justify-center text-sm">No image</div>
          )}
          <div>
  ...
  
  {
    accessorKey: "price",
    header: "Price (AED)",
    cell: ({ row }) => <p className="text-14-regular">AED {row.original.price ?? "—"}</p>,
  },
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex gap-2 items-center">
          ...
          >
            <PropertyForm realtorId={item.realtor_id} onSuccess={() => window.location.reload()} />
          </FormDialog>
